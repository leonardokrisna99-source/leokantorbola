...
# ==========================
# Mapping Nama (ISI SENDIRI)
# ==========================
# Kiri = nama asli (jadwal), kanan = nama tampilan di klasemen
name_map = {
    "Leo": "Masjawa2",
    "Irzal": "Wenger",
...
# ==========================
# Load or Create Data (Persistent)
# ==========================
...
for _, row in matches_week.iterrows():
    ...
    with c2:
        hs = st.text_input(f"hs_{row['match_id']}", value=("" if pd.isna(row['home_score']) else str(int(row['home_score']))), key=f"hs_{row['match_id']}")
    with c3:
        aw = st.text_input(f"aw_{row['match_id']}", value=("" if pd.isna(row['away_score']) else str(int(row['away_score']))), key=f"aw_{row['match_id']}")
    with c4:
        if st.button("Simpan", key=f"save_{row['match_id']}"):
            try:
                ...
                st.error("Skor harus angka!"); st.stop()
...
for idx,row in standings.iterrows():
    ...
    elif idx==1: cls="top2"; medal="🥈"
    elif idx==2: cls="top3"; medal="🥉"
    if idx >= n-3: cls+=" bottom3"
    bar_width = int((row['Poin']/max_poin)*100) if max_poin>0 else 0

    # Nama klasemen pakai mapping
    display_name = name_map.get(row['Team'], row['Team'])

    bottom_emojis=["😵‍💫","🤯","🪦"]
    bottom_emoji = bottom_emojis[idx-(n-3)] if idx>=n-3 else ""

    html += f"<tr class='{cls}'>"
    html += f"<td>{idx+1}</td><td>{medal}{bottom_emoji} {display_name}</td><td>{row['Main']}</td><td>{row['Menang']}</td><td>{row['Seri']}</td><td>{row['Kalah']}</td>"
    html += f"<td>{row['GF']}</td><td>{row['GA']}</td><td>{row['GD']}</td>"
